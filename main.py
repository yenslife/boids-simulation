...
ADD_BOID_BUTTON_RECT = pygame.Rect(200, 10, 100, 40)
REMOVE_BOID_BUTTON_RECT = pygame.Rect(310, 10, 100, 40)
EAT_MODE_BUTTON_RECT = pygame.Rect(420, 10, 180, 40)
OBSTACLE_MODE_BUTTON_RECT = pygame.Rect(610, 10, 220, 40)
...
class Boid:
    def __init__(self):
        ...
        self.velocity = pygame.math.Vector2(math.cos(angle), math.sin(angle)) * random.uniform(MIN_SPEED, MAX_SPEED)
...
class Predator(Boid):
    def __init__(self):
        super().__init__()
        self.velocity = pygame.math.Vector2(random.uniform(-1, 1), random.uniform(-1, 1)).normalize() * PREDATOR_SPEED
        self.can_eat = False
    ...
    def draw(self, screen):
        ...
        p2 = self.position + pygame.math.Vector2(-PREDATOR_SIZE, PREDATOR_SIZE).rotate(-angle)
        p3 = self.position + pygame.math.Vector2(-PREDATOR_SIZE, -PREDATOR_SIZE).rotate(-angle)
...
def main():
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    if PREDATOR_BUTTON_RECT.collidepoint(mouse_pos):
                        ...
                        mouse_mode = 'remove_obstacle' if mouse_mode == 'add_obstacle' else 'add_obstacle'
                        clicked_on_ui = True

                    if not clicked_on_ui:
                        if mouse_mode == 'add_obstacle':
                            obstacles.append({'center': pygame.math.Vector2(mouse_pos), 'radius': OBSTACLE_RADIUS})
                        elif mouse_mode == 'remove_obstacle':
                            for obs in obstacles[:]:
                                if pygame.math.Vector2(mouse_pos).distance_to(obs['center']) < obs['radius']:
        ...
        # --- 繪製 UI ---
        pred_button_color = BUTTON_HOVER_COLOR if PREDATOR_BUTTON_RECT.collidepoint(mouse_pos) else BUTTON_COLOR
        pygame.draw.rect(screen, pred_button_color, PREDATOR_BUTTON_RECT, border_radius=5)
        ...
        screen.blit(pred_text, pred_text.get_rect(center=PREDATOR_BUTTON_RECT.center))

        add_button_color = BUTTON_HOVER_COLOR if ADD_BOID_BUTTON_RECT.collidepoint(mouse_pos) else BUTTON_COLOR
        pygame.draw.rect(screen, add_button_color, ADD_BOID_BUTTON_RECT, border_radius=5)
        add_text = font.render("+ Boid", True, BUTTON_TEXT_COLOR)
        screen.blit(add_text, add_text.get_rect(center=ADD_BOID_BUTTON_RECT.center))

        remove_button_color = BUTTON_HOVER_COLOR if REMOVE_BOID_BUTTON_RECT.collidepoint(mouse_pos) else BUTTON_COLOR
        pygame.draw.rect(screen, remove_button_color, REMOVE_BOID_BUTTON_RECT, border_radius=5)
        remove_text = font.render("- Boid", True, BUTTON_TEXT_COLOR)
        screen.blit(remove_text, remove_text.get_rect(center=REMOVE_BOID_BUTTON_RECT.center))

        eat_button_color = BUTTON_HOVER_COLOR if EAT_MODE_BUTTON_RECT.collidepoint(mouse_pos) else BUTTON_COLOR
        pygame.draw.rect(screen, eat_button_color, EAT_MODE_BUTTON_RECT, border_radius=5)
        ...
        screen.blit(eat_text, eat_text.get_rect(center=EAT_MODE_BUTTON_RECT.center))

        obstacle_button_color = BUTTON_HOVER_COLOR if OBSTACLE_MODE_BUTTON_RECT.collidepoint(mouse_pos) else BUTTON_COLOR
        pygame.draw.rect(screen, obstacle_button_color, OBSTACLE_MODE_BUTTON_RECT, border_radius=5)
        obstacle_text_str = "Mouse: Remove Obstacle" if mouse_mode == 'remove_obstacle' else "Mouse: Add Obstacle"
        obstacle_text = font.render(obstacle_text_str, True, BUTTON_TEXT_COLOR)
        screen.blit(obstacle_text, obstacle_text.get_rect(center=OBSTACLE_MODE_BUTTON_RECT.center))
